...
static uint32_t getUserDefinedSectionsSummarySize(AsmEncoder* as)
{
    ...
    uint32_t size = 0;
    for (size_t i = 0; i < as->parser->sectionsSz; i++)
        size += (uint32_t)as->parser->sections[i].size;
...
{
    uint32_t     symTabSz = as->parser->symsData.symTabSz;
...
{
    uint32_t userSectsBodyOffset =
        sizeof(BinformatHeader) + (uint32_t)sizeof(SectionHeader) * getSectionsCount(as);
    ...
    for (; usrSectIdx < as->usrSectionCount; usrSectIdx++)
    {
        ...
        currUsrSect->size   = (uint32_t)currUsrSectNode->size;
        ...
        uint32_t sectRelocsCount =
            getRelocationsCountInSection(&as->parser->symsData, currUsrSectNode->name);

        if (sectRelocsCount == 0)
            continue;

        printf("section %s: %u relocations at file offset %u\n", currUsrSectNode->name,
               sectRelocsCount, relocSectsBodyOffset);
...
{
    printf("writing strtab at offset: %ld ; size: %lu\n", ftell(as->out),
           sizeof(char) * as->strTabSize);
    fwrite(as->strTab, sizeof(char), as->strTabSize, as->out);
}

static void writeSymTab(AsmEncoder* as)
{
    printf("writing symtab at offset: %ld ; size: %lu\n", ftell(as->out),
           sizeof(SymTabEntry) * as->symTabSz);
...
static void writeRelocForSection(AsmEncoder* as, const char* sectName)
{
    for (size_t i = 0; i < as->parser->symsData.symbolsSz; i++)
    {
        ...
        printf("added relocation for section %s symbol: %s at file offset %ld\n", sectName,
               currSymb->name, offset);
    }
}
